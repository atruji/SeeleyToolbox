...
set(handles.fslReStOn,'Value',1)
handles.opt.fsl_ReSt='fslReStOn';
...
% --- Executes when selected object is changed in ReSt.
function ReSt_SelectionChangeFcn(hObject, eventdata, handles)
% hObject    handle to the selected object in ReSt 
% eventdata  structure with the following fields (see UIBUTTONGROUP)
%	EventName: string 'SelectionChanged' (read only)
%	OldValue: handle of the previously selected object or empty if none was selected
%	NewValue: handle of the currently selected object
% handles    structure with handles and user data (see GUIDATA)
handles.opt.fsl_ReSt=get(hObject,'Tag');
if strcmp(handles.opt.fsl_ReSt,'fslReStOff')
    set(handles.IcaFullOff,'Value',1)
    handles.opt.IcaFull='IcaFullOff';
    set(handles.ileaveOff,'Value',1)
    handles.opt.ileave='ileaveOff';
else
    set(handles.IcaFullOn,'Value', 1) 
